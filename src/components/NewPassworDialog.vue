  ...
  <q-dialog v-model="triggered">
    <q-card class="row">
      <q-card-section class="rowContainer q-pb-none col-12">
        <div class="text-h6">{{ $t('change_passwd') }}</div>
        <q-space />
      ...
      <q-card-section class="q-pa-md columnContainer justify-center col-12 row">
        <q-form class="columnContainer justify-center col-12 row">
          <PasswordInput :placeholder="$t('old_pswd')" icon="lock_open" :tooltip="$t('old_pswd')" v-model="oldPassword"
            :isVisible='true' />
          <PasswordInput :placeholder="$t('new_pswd')" icon="lock" :tooltip="$t('new_pswd')" v-model="newPassword"
            :isVisible='false' :isEmail="false" />
        </q-form>
        <q-btn color="teal" flat :label="$t('change_passwd')" class="item-small col-5 q-pa-md" v-close-popup
